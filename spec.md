...
- プレースホルダー置換機能
- 複数形対応（Pluralization）- Intl.PluralRules API 基盤
- TypeScript による型安全性
...
- 複数形のサフィックス（`_one`, `_other`, `_few`, `_many` など Intl.PluralRules カテゴリ）を含む場合もある
...
- **注意**: 複数形を使用する場合、基本キー（例: `"itemCount"`）のみを指定し、サフィックス付きキーは自動的に解決される
...
**サフィックスの種類（Intl.PluralRules カテゴリ）:**

- `_one`: 単数形（例: 英語で count = 1）
- `_other`: 複数形・デフォルト（必須、フォールバックとして使用）
- `_zero`: ゼロ（一部の言語で使用、オプショナル）
- `_two`: 2（一部の言語で使用、オプショナル）
- `_few`: 少数（一部の言語で使用、オプショナル）
- `_many`: 多数（一部の言語で使用、オプショナル）

`_other` のみが必須で、他のカテゴリはロケールの複数形ルールに応じて任意に追加できます。
...
**複数形ルール選択ロジック（Intl.PluralRules 基盤）:**

1. `Intl.PluralRules` API を使用して、count に対応するカテゴリを取得（例: "one", "other"）
2. カテゴリに対応するキー（例: `key_one`, `key_other`）を検索
3. 該当するキーが見つからない場合、`key_other` にフォールバック（必須）
4. `key_other` も存在しない場合、キー名をそのまま返す

**ロケールごとの複数形ルール例:**
- 英語: `one` (1), `other` (0, 2, 3, ...)
- 日本語: `other` (すべての数値)
- アラビア語: `zero` (0), `one` (1), `two` (2), `few` (3-10), `many` (11-99), `other` (100+)
...
4. **複数形キーの自動解決**: 基本キー（例: `"itemCount"`）が指定された場合、Intl.PluralRules カテゴリに基づくサフィックス付きキー（`_one`, `_other`, `_few` など）も自動的に抽出
...
// 出力 Messages（存在するキーのみ）
{
  'common.itemCount_one': '{{count}}件のアイテム',
...
2. `count` が存在する場合、`Intl.PluralRules` を使用して複数形カテゴリを決定し、適切なキーを選択
...
- 複数形キーが見つからない場合: 選択されたカテゴリのキーが存在しなければ `_other` にフォールバック、それもなければ基本キーを返す
...
**目的:** `Intl.PluralRules` を使用して数値に応じた複数形キーのサフィックスを選択

**引数:**

- `baseKey`: 基本キー名（例: `"itemCount"`）
- `count`: 判定する数値
- `locale`: ロケール（オプション、デフォルト: `"en"`）
...
const category = pluralRules.select(count); // "one", "other", "few", etc.
...
1. `selectPluralKey` で `Intl.PluralRules` を使用して適切なカテゴリとキーを選択
...
例:
```
count = 0 (英語): itemCount_other (Intl.PluralRules で "other" が返される)
count = 1 (英語): itemCount_one (Intl.PluralRules で "one" が返される) → _other → undefined
count = 5 (英語): itemCount_other (Intl.PluralRules で "other" が返される) → undefined
```
...
- 存在する複数形キーの配列（例: `["itemCount_one", "itemCount_other"]` または `["itemCount_zero", "itemCount_one", "itemCount_few", "itemCount_other"]`）

**動作:**

1. `namespace` 内で Intl.PluralRules がサポートするすべてのカテゴリ（`_zero`, `_one`, `_two`, `_few`, `_many`, `_other`）を検索
