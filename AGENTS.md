...
- **`Locale` is a closed union** (`"en" | "ja"`) in `src/types.ts`. Adding a locale means editing that type; nothing infers it from disk.
...
- `check` first attempts multi-locale mode on the given path and silently falls back to single-locale-directory mode on failure.
